...
(function (root) {
  ...
  function verifyFeed(text, target, date) {
    ...
    if (n > 1) {
      return { type: "feed", status: "failed",
               detail: n + " different checkpoints published in the feed for these "
                       + "days — the journal was forked" };
    }
    if (found) {
      return { type: "feed", status: "ok", placed: true,
               detail: "entry " + found.seq + " in the feed; the strength of this "
                       + "seal depends on whether the feed is published outside" };
    }
    if (sameDate.length) {
      return { type: "feed", status: "failed",
               detail: "a DIFFERENT checkpoint is published in the feed for these "
                       + "days (" + sameDate[0].target.slice(0, 16)
                       + "…) — the journal does not match it" };
    }
  ...
  function anchorBlob(name, data, target, date) {
    ...
    if (/\.rfc3161\.tsr$/.test(base)) {
      ...
      return contains(data, target)
        ? { type: "rfc3161", status: "unverified", placed: true,
            detail: "the timestamp covers this checkpoint; the authority "
                    + "signature is checked by aihash-verify" }
    ...
    return { type: base, status: "unverified",
             detail: "unknown seal type — not counted" };
  }
  ...
  function verify(bytes) {
    return loadBundle(bytes).then(function (files) {
      ...
      function need(name) {
        if (!files[name]) throw new Error("the bundle has no " + name);
      ...
      if (!ok("format version", manifest.format === "aihash/1", manifest.format))
        return out;

      for (var i = 0; i < records.length; i++) {
        var rec = records[i], p = proofs.records[String(rec.seq)];
        if (!p) { ok("record seq=" + rec.seq + ": путь в пакете", false); return out; }
        var croot;
        try { croot = contentRoot(rec.fields); }
        catch (e) { ok("record seq=" + rec.seq + ": поля", false, e.message); return out; }
        var l = link(unhex(p.prev_link), rec.seq, croot, rec.ts);
        if (!ok("record seq=" + rec.seq + ": content and link", hex(l) === p.link))
          return out;
        var got = applyPath(segLeaf(l), p.segment_path);
        if (!ok("record seq=" + rec.seq + ": path to the segment root",
                hex(got) === segment.segment_root)) return out;
      }

      var ck;
      try { ck = segCkpt(segment); }
      catch (e) { ok("segment checkpoint", false, e.message); return out; }
      if (!ok("segment checkpoint", hex(ck) === segment.checkpoint)) return out;

      var droot = applyPath(dayLeaf(ck), proofs.day_path);
      if (!ok("path to the daily root", hex(droot) === day.day_root)) return out;

      var dck = dayCkpt(day);
      if (!ok("daily checkpoint", hex(dck) === day.day_checkpoint)) return out;

      out.anchors = verifyAnchors(files, dck, day.date);
      for (var j = 0; j < out.anchors.length; j++) {
        if (out.anchors[j].status === "failed")
          ok("seal " + out.anchors[j].type, false, out.anchors[j].detail);
      }
